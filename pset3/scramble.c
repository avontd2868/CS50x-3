...
{  
    ...
    while (low <= high) 
    {
        ...
        {   
            // Checks to see if the world has been found
        ...
        // Checks to see if the word is greater than the midpoint        
        else if (strcmp(s, dictionary.words[mid].letters) > 0)
            low = mid + 1;
   ...
   //Tried to use recursion but don't fully understand the concept.
   /* if (dictionary.size <= 0) 
        return false;
    if ((strcmp(s, dictionary.words[WORDS/2].letters) == 0 ) && (dictionary.words[WORDS/2].found != true)) 
        return true;
    else if (strcmp(s, dictionary.words[WORDS/2].letters) > 0 )
        return ((((dictionary.size/2) + 1)) + lookup(s)); 
    else //(strcmp(s, dictionary.words[WORDS/2].letters) < 0 )
        return ((((dictionary.size/2) - 1)) + lookup(s));
    //else
     //   return false; */ 
}
